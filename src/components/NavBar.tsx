  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-dark shadow-md border-b border-gray-800 z-50" role="navigation" aria-label="Main navigation">
      <div className="container mx-auto flex justify-between items-center py-4">
        ...
        <div className="hidden md:flex items-center space-x-6">
          {navLinks.map(link => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative text-light-gray transition-colors hover:text-dark-accent ${
                isActive(link.href)
                  ? 'text-dark-accent after:absolute after:-bottom-1 after:left-0 after:w-full after:h-0.5 after:bg-dark-accent'
                  : ''
              }`}
            >
              {link.label}
        ...
        {/* Mobile Toggle */}
        <button
          className="md:hidden text-light-gray p-2 focus:outline-none"
