...
export default function GalleryGrid({ onImageClick }: { onImageClick: (image: string) => void }) {
  ...
  return (
    <section className="py-12">
      <h2 className="text-3xl font-bold text-center mb-6 text-text-base">
        Moments Frozen in Time
      </h2>
      <div className="flex justify-center mb-6 items-center gap-2">
        <label htmlFor="category-filter" className="text-text-base font-medium">
          Filter by Category:
        </label>
        <select
          id="category-filter"
          ...
          className="px-4 py-2 rounded text-text-base bg-neutral-mid ring-1 ring-white/10 focus:ring-2 focus:ring-accent"
        >
          {categories.map(cat => (
